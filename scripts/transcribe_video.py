...
def _build_transcription_cache(input_file, model_name, srt_file, tsv_file, json_file):
...
def _cache_matches(cache, input_file, model_name, srt_file, tsv_file, json_file):
    if not cache:
        return False
    try:
        stat = os.stat(input_file)
...
def _save_transcription_cache(cache_path, input_file, model_name, srt_file, tsv_file, json_file):
    try:
        cache = _build_transcription_cache(input_file, model_name, srt_file, tsv_file, json_file)
...
def transcribe(input_file, model_name='large-v3', project_folder='tmp'):
    ...
    if _cache_matches(cache, input_file, model_name, srt_file, tsv_file, json_file):
